...
func Test_Service_sends_only_last_chunk_with_sound(t *testing.T) {
	t.Parallel()
...
func Test_Service_sorts_pending_books_by_publication_year_before_chunking(t *testing.T) {
	...
	messageLimit := 60
	expectedChunks, err := renderChunks(t, expectedOrder, digest.Options{LocalTime: now, Limit: messageLimit})
	require.NoError(t, err)
	state := &fakeState{pending: books, recordedNew: len(books)}
	sender := &fakeSender{}
	service := app.NewService(app.Dependencies{
	...
	require.True(t, sender.silent[0])
...
func Test_Service_sends_later_chunk_that_fits_only_without_header(t *testing.T) {
	...
	expectedChunks, err := renderChunks(t, books, digest.Options{Limit: messageLimit})
	require.NoError(t, err)
...
func headerlessOnlyLaterBookFixture(t *testing.T) ([]alib.Book, int) {
	...
	firstBookChunks, err := renderChunks(t, books[:1], digest.Options{Limit: 4096})
	require.NoError(t, err)
	messageLimit := displayedRuneCount(t, firstBookChunks[0].Text)
	secondBookChunks, err := renderChunks(t, books[1:], digest.Options{Limit: 4096})
	require.NoError(t, err)
...
func Test_Service_sends_renderable_pending_books_when_one_pending_book_is_too_long(t *testing.T) {
	...
	firstChunks, err := renderChunks(t, []alib.Book{firstDeliverable}, digest.Options{Limit: 4096})
	require.NoError(t, err)
	...
	// Then
	require.NoError(t, err)
	require.Equal(t, app.Result{Sent: 2, Failed: 1}, result)
...
func (f *fakeState) Existing(_ context.Context, books []alib.Book) ([]bool, error) {
	if f.existing == nil {
