...
func Test_NewClient_rejects_negative_request_interval(t *testing.T) {
	...
	// When
	client, err := alib.NewClient("https://www.alib.ru/tramka.phtml?tnew=7", time.Second, -time.Second, nil)
...
func Test_Client_fetches_urls_in_order_and_deduplicates_by_buy_url(t *testing.T) {
	...
	rawURLs := strings.Join([]string{
		" " + server.URL + "/first?one=1&two=2",
		server.URL + "/second?query=a%2Cb ",
		server.URL + "/third?last=true",
	}, ", ")
	...
	require.Equal(t, []alib.Book{
		{Title: "First", Price: "100 руб.", BuyURL: server.URL + "/book-1.html"},
		{Title: "Second", Price: "200 руб.", BuyURL: server.URL + "/book-2.html"},
		{Title: "Third", Price: "300 руб.", BuyURL: server.URL + "/book-3.html"},
		{Title: "Fourth", Price: "400 руб.", BuyURL: server.URL + "/book-4.html"},
	}, books)
}
