...
- [ ] Обновить тест стандартной конфигурации: `MESSAGE_LIMIT` по умолчанию равен 32000.
- [ ] Добавить граничные тесты: значение 32768 принимается, 32769 и значения ниже 64 отклоняются с ошибкой, называющей `MESSAGE_LIMIT`.
- [ ] Изменить `defaultMessageLimit` на 32000 и `telegramHardMessageLimit` на официальный максимум 32768.
- [ ] Обновить Compose default до 32000, сохранив возможность пользовательского переопределения.
- [ ] Запустить `make test`; команда должна пройти до Task 2.
...
- [ ] Добавить тесты счётчика: HTML-сущность считается как одна руна, текст ссылки учитывается, `href` и форматирующие теги не учитываются, `<br/>` считается переносом, `<hr/>` не добавляет текст.
- [ ] Реализовать один закрытый helper на базе `golang.org/x/net/html`, считающий декодированные текстовые руны и переносы в сгенерированном Rich HTML.
- [ ] Заменить сырой `utf8.RuneCountInString` этим helper во всех проверках заголовка, отдельной книги, текущего чанка и кандидатов бинарного поиска.
- [ ] Сохранить запас `MESSAGE_LIMIT - 1` для сокращённого описания и максимальный подходящий префикс перед `…`.
- [ ] Обновить существующие граничные тесты чанкинга и сокращения, чтобы их лимиты задавались в рунах отображаемого текста.
- [ ] Проверить, что слишком длинные обязательные текстовые поля по-прежнему дают `ErrMessageTooLong`, а другие книги отправляются.
- [ ] Запустить `make test`; команда должна пройти до Task 3.
...
- [ ] Сохранить минимальный UTF-8 HTML-фрагмент только проблемного объявления, без полной скачанной страницы.
- [ ] Разобрать fixture существующим `alib.Parse` и проверить точный `BuyURL` и наличие 16 фотографий.
- [ ] Отрендерить книгу с лимитом 4000 и проверить отсутствие `ErrMessageTooLong`; это более строгое условие, чем новый default 32000.
- [ ] Проверить, что сырой HTML длиннее 4000 рун, но отображаемый Rich Message текст не превышает лимит.
- [ ] Проверить сохранение полного описания, всех обязательных полей, всех ссылок на фотографии и исходной книги в `Chunk.Books`.
- [ ] Дополнительно отрендерить книгу с новым стандартным лимитом 32000, подтверждая штатный производственный сценарий.
- [ ] Запустить `make test`; команда должна пройти до Task 4.

### Task 4: Verify acceptance criteria

- [ ] Запустить полный `make verify`.
- [ ] Запустить `make coverage` и подтвердить общее покрытие не ниже 80%.
- [ ] Проверить `docker compose config` с новым стандартным `MESSAGE_LIMIT`.
- [ ] Подтвердить регрессионным тестом, что книга Беляева отправляема при лимитах 4000 и 32000.
- [ ] Подтвердить, что реальное превышение отображаемого лимита сокращает только `Content`.
- [ ] Подтвердить сохранение `ErrMessageTooLong` для непомещающихся обязательных текстовых полей.
