...
- [ ] Добавить тесты отсутствующего и пустого `FRESH_BOOKS`: конфигурация успешно
  загружается, порог `✨` отключён.
- [ ] Добавить типизированную необязательную настройку `FRESH_BOOKS`.
- [ ] Для `age:N` принимать неотрицательное целое; нижняя включительная граница равна
  `текущий год - N`.
- [ ] Для `since:YYYY` принимать четырёхзначный включительный нижний год.
- [ ] Не назначать `age:5` по умолчанию: без указанного порога действует только логика `🔥`.
- [ ] Добавить тесты режимов `age:N`, `since:YYYY`, `age:0` и включительных границ.
- [ ] Добавить Compose passthrough `${FRESH_BOOKS:-}` без автоматического порога.
...
- [ ] Добавить parser-тесты объявлений с содержанием, без содержания, без продавца, с
  фото, ISBN, `2026г.`, `1970 г.`, дубликатом и относительными URL.
- [ ] Заменить три несемантических текстовых фрагмента полями: заголовок,
...
- [ ] Построить логические строки обходом DOM; `<br>` задаёт границы, вложенные
  text/anchor-узлы сохраняют порядок.
- [ ] Основным блоком считать заголовок и библиографический текст до строки продажи;
  ISBN оставить в этом блоке.
- [ ] Разобрать строку продажи по seller-anchor, `Цена:` и purchase-anchor; поддержать объявления
  без seller-anchor.
- [ ] Удалить только служебный префикс `BS - ` из отображаемого имени, сохранив seller URL.
- [ ] После purchase-anchor считать строки до первой `Состояние:` содержанием; `Состояние:` и
  последующий непропущенный текст — остальными сведениями.
- [ ] Исключить `Смотрите:`/photo anchors из текста и сохранить `HasPhotos`.
- [ ] Извлекать последний валидный год из основной строки сканированием цифр и
  суффикса `г`/`г.`, без regex; ISBN и годы внутри содержания игнорировать.
- [ ] Сохранить deduplication по разрешённому `BuyURL` и `alib.ErrNoBooks`.
...
- [ ] Добавить точные HTML-тесты для текущего года, января, предыдущего года,
  отключённого порога, обоих threshold-режимов, включительной нижней границы, старой
  книги, будущего и отсутствующего года.
- [ ] Передавать рендереру limit, локальное время цикла и необязательную
  нормализованную нижнюю границу года через единые render options.
- [ ] Добавлять `🔥 ` перед `<b>...</b>` для текущего года и предыдущего года в январе.
- [ ] При настроенном пороге добавлять `✨ ` для остальных годов между включительной
  нижней границей и текущим годом.
- [ ] При отключённом пороге не добавлять `✨`.
- [ ] Применять январское правило раньше проверки настроенного порога.
- [ ] Формировать объявление в порядке:
  ...
  6. пустая строка;
  7. ссылка `Купить`.
- [ ] Форматировать продавца как `Продавец: <a href="...">BotSad</a>, Москва.` и цену как
  `Цена: 3900 руб.` на отдельных строках.
- [ ] При отсутствии содержания отделять основную строку пустой строкой непосредственно
  от seller/price/details блока.
- [ ] Не создавать лишние пустые абзацы при отсутствии необязательных полей.
- [ ] Сохранить HTML escaping текста и URL, полный source text, rune-based limits и
  разбиение только между книгами.
- [ ] Обновить chunking и `ErrMessageTooLong` тесты с учётом emoji и новых переносов.
