...
- [ ] Изменить `executeJob`, чтобы он возвращал `app.Result` вместе с ошибкой, включая частичный результат при сбое.
- [ ] Сохранить закрытие bbolt, объединение ошибок закрытия и существующий `digest.completed` log.
- [ ] Адаптировать once/startup/scheduled пути: once возвращает только ошибку наружу, остальные игнорируют результат.
- [ ] Добавить для refresh completion hook, который вызывается после полного завершения `executeJob` и получает итоговые `app.Result` и `error`.
- [ ] Сохранить общий runner lock, асинхронный refresh и `BeforeDelivery`.
- [ ] Обновить process/runner-тесты: проверить передачу успешного результата, частичного результата, ошибки и ошибки закрытия state.
...
- [ ] Убрать ранний toast `Проверяю новые книги`, чтобы Telegram сохранял `Loading...` во время проверки.
- [ ] После успешного refresh с `Result.New == 0` вызвать `AnswerCallback` с текстом `Новых книг нет`.
- [ ] После успешного refresh с `Result.New > 0` вызвать `AnswerCallback` с пустым текстом.
- [ ] При ошибке вызвать `AnswerCallback` с `err.Error()`, сохранив очистку Telegram-ошибок от bot token.
- [ ] Сохранить `digest.failed` для ошибок digest и `callback.answer_failed` для ошибок отправки итогового callback.
- [ ] Сохранить немедленные ответы `Проверка уже выполняется` и `Кнопка недоступна`; неизвестные callback data по-прежнему игнорировать.
- [ ] Расширить callback-тесты: до завершения блокирующего fetch ответа нет; пустой успешный результат даёт `Новых книг нет`; найденная книга даёт пустой ответ; ошибка даёт её текст.
- [ ] Проверить тестами, что удаление кнопки и отправка сохраняют прежние условия и порядок, а итоговый callback отправляется после digest.
- [ ] Обновить README и runtime invariants в AGENTS.md: описать три итоговых статуса и указать, что время показа toast контролирует Telegram.
