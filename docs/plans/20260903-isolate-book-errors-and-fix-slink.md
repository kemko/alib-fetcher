...
- [x] Удалить `refreshCallbackTimeout` и создание 10-секундного контекста.
- [x] После успешного захвата runner lock сразу отвечать callback текстом `Формирование дайджеста запущено`.
- [x] Продолжать digest в фоне на контексте жизненного цикла сервиса; сохранить отмену при shutdown и ожидание refresh goroutine.
- [x] Не отправлять второй callback-answer после завершения digest; итог и ошибки оставлять в `digest.completed`/`digest.failed`.
- [x] Сохранить ответы для чужого чата и уже выполняющегося digest, polling во время фоновой работы и удаление старой кнопки только перед первой новой отправкой.
- [x] Добавить тесты немедленного ответа, работы дольше прежних 10 секунд, ошибки фонового digest, duplicate callback и shutdown cancellation.
- [x] Выполнить `make test`; исправить все ошибки до Task 6.
