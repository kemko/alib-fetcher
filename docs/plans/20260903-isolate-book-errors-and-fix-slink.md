...
- [ ] Добавить результат fetch/parse с успешно разобранными книгами и числом ошибок конкретных объявлений.
- [ ] Отличать посторонний `<p>` от похожего на объявление, которое не удалось разобрать; malformed-объявление пропускать без ошибки всей страницы.
- [ ] Сохранить `ErrNoBooks` для пустой или структурно изменившейся страницы, не распознанной как корректная выдача.
- [ ] Дедуплицировать ошибки по разрешённому `BuyURL`; если та же книга успешно разобрана на другой странице, не считать её сбойной.
- [ ] Сохранить порядок, межстраничную дедупликацию и существующую политику частичного успеха страниц.
- [ ] Добавить тесты смешанной страницы, страницы только со сбойными объявлениями, дублей ошибок и успешного дубля после ошибки.
...
- [ ] Добавить валидацию `SLINK_API_KEY` по обязательному префиксу `sk_`, сохранив имя переменной в ошибке и исключив значение ключа из логов.
- [ ] Реализовать точный контракт Slink: `POST /api/external/upload`, Bearer auth, multipart `image`, `tagIds[]`, `Origin` из настроенного Slink base URL и разбор поля `url`.
- [ ] Для загрузки исходных фотографий передавать стабильный User-Agent и безопасный Referer страницы книги.
- [ ] Возвращать первую ошибку download, HTTP/META redirect, распознавания изображения или Slink upload как ошибку всей книги; корректный non-image сохранять исходной ссылкой.
- [ ] Сохранить терминальность отмены контекста и гарантированную очистку временного каталога.
- [ ] Сделать `slink.photo_failed` диагностичным без утечки URL и ключа: добавить этап, HTTP status и безопасную категорию ошибки, различающие source 403/502 и Slink 403/502.
- [ ] Добавить тесты заголовков, API-key validation, multipart-контракта, ошибок каждого этапа, безопасных логов и очистки после частичного выполнения.
...
- [ ] Передавать renderer число ошибок предыдущих этапов и прибавлять книги, пропущенные из-за `ErrMessageTooLong`.
- [ ] Если итоговое число больше нуля, добавлять в конец последней части дайджеста секцию `Не удалось обработать книг: N`, отделённую от секций книг через `<hr/>`.
- [ ] Не добавлять секцию ошибок при нулевом числе сбоев.
- [ ] При отсутствии успешно обработанных книг формировать digest из заголовка и секции ошибок.
- [ ] Учитывать секцию и разделитель в text/block limits; при необходимости формировать дополнительную последнюю часть с сохранением секции.
- [ ] Сохранить media limits, атомарность книг, порядок chunks и список `BuyURL` книг, пропущенных renderer.
- [ ] Добавить тесты одного и нескольких сбоев, нулевого числа сбоев, summary-only digest, split на границе лимита и oversized-книги.
...
- [ ] Добавить read-only проверку текущих `BuyURL`, чтобы выделять новые книги без предварительной записи в БД и сохранять source order.
- [ ] Последовательно подготовить фотографии каждой новой книги и очистить её временный каталог до перехода к следующей.
- [ ] При book-specific ошибке накопить один сбой и продолжить с остальными книгами; отмену контекста и общую ошибку состояния завершать глобальной ошибкой.
- [ ] До записи выполнить индивидуальную проверку рендеринга и исключить слишком длинные книги.
- [ ] Передавать в `RecordDiscovered` только полностью подготовленные и renderable книги.
- [ ] При обработке набора для отправки исключать книгу с локальной ошибкой только из текущего дайджеста и не прерывать остальные операции.
- [ ] Отправлять подготовленные chunks и подтверждать только книги соответствующего успешно отправленного chunk.
- [ ] Добавить `Failed` в `app.Result` и поле `failed` в `digest.completed`; book-specific ошибки не должны создавать `digest.failed`.
- [ ] Добавить тесты смешанного успеха, отсутствия новой сбойной книги в БД, её повторного обнаружения на следующем запуске, корректного `New/Failed/Sent`, Telegram failure и отмены контекста.
