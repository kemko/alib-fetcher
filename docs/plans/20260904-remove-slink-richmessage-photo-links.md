...
- [ ] Удалить `SlinkProfile` из renderer options, генерацию `<tg-slideshow>`, `<img>` и `<figcaption>`, а также media-limit
  расчёты.
- [ ] Всегда выводить каждую `Book.Photos` как исходную ссылку в единственном блоке
  `Смотрите`, сохраняя подпись, порядок и повторы; пустую подпись заменять на `фото`.
- [ ] Упростить подсчёт Rich Message blocks до обычных listing-блоков и `<hr/>`, сохранив границу 500
  блоков и максимум 250 объявлений в chunk.
- [ ] Сохранить расчёт `MESSAGE_LIMIT` по отображаемым Unicode-рунам, default `32000`, failure summary и
  разбиение только между книгами.
- [ ] Сохранить существующее сокращение длинного `Content`: выбрать максимально
  допустимый префикс и завершить его `…`, чтобы книга поместилась в сообщение.
- [ ] Не считать длинный `Content` ошибкой подготовки: новая книга должна записываться,
  отправляться в сокращённом виде и не увеличивать счётчик сбоев.
- [ ] Оставить `digest.ErrMessageTooLong` только для книги, обязательные отображаемые поля
  которой превышают лимит даже при полностью удалённом `Content`.
- [ ] Удалить `PhotoProcessor`, подготовку фотографий, временную очистку и `SavePrepared` из
  app-интерфейса и digest lifecycle.
- [ ] Удалить создание Slink-клиента из bootstrap; `run` должен напрямую собирать сервис без
  фабрики обработки фотографий.
- [ ] Обновить renderer/service tests: все исходные ссылки присутствуют в `Смотрите`, slideshow
...
- [ ] Обновить bootstrap tests, подтверждающие вызов `/sendRichMessage`, передачу HTML через `rich_message.html`
...
- [ ] Добавить или обновить end-to-end regression test: Alib-объявление с несколькими фотоссылками
  отправляется через `/sendRichMessage`, все исходные ссылки и подписи находятся в `Смотрите`,
  а slideshow/media отсутствуют.
- [ ] Добавить end-to-end случай с описанием длиннее лимита: описание сокращается с `…`,
  книга записывается и отправляется, `failed` не увеличивается, отображаемый текст не
  превышает лимит.
- [ ] Зафиксировать тестом default `MESSAGE_LIMIT=32000` и разбиение Rich Message без перехода на
  обычный Telegram `sendMessage`.
- [ ] Удалить из README конфигурацию, сетевой поток, временные файлы, persistence и
  эксплуатационные требования Slink.
- [ ] Описать единственный фотоформат: блок `Смотрите` со всеми исходными ссылками,
  подписями, порядком и повторами.
- [ ] Описать сокращение только длинного описания с `…` при сохранении и отправке
  книги.
- [ ] Обновить AGENTS.md: runtime flow, invariants, модель состояния, digest limits, repository map, configuration contract,
