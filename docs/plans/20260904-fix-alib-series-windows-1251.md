...
- [ ] В `buildAlibURLs` кодировать каждое значение серии через существующий Windows-1251 encoder до вызова
  `url.QueryEscape`.
- [ ] Оборачивать ошибку непредставимого символа в `config.ErrInvalid`, называя `ALIB_SERIES` и проблемный
  элемент.
- [ ] Сохранить существующее формирование URL, порядок категорий и серий, повторы, CSV-значения и `lday=7`.
- [ ] Добавить регрессионный случай `отцы основатели` с ожидаемым query
  `seria=%EE%F2%F6%FB+%EE%F1%ED%EE%E2%E0%F2%E5%EB%E8&lday=7`.
- [ ] Обновить ожидаемые URL для существующих кириллических серий; сохранить проверки пробелов, запятых, `&`,
  `/` и ASCII.
- [ ] Добавить проверку отказа для символа вне Windows-1251 без частично сформированной конфигурации.
- [ ] Обновить интеграционный тест запуска: проверить Windows-1251 `RawQuery`, корректное смысловое значение после
  декодирования и полный URL в page logs.
- [ ] Выполнить `make test`; весь набор тестов должен пройти до Task 2.
...
- [ ] Уточнить, что `ALIB_SERIES` вводится как обычный Unicode CSV, но каждое значение должно представляться в
  Windows-1251.
- [ ] Описать, что query-параметр `seria` содержит percent-encoded байты Windows-1251, поскольку этого ожидает
  Alib.ru.
- [ ] Зафиксировать ошибку конфигурации для непредставимых символов.
- [ ] Не менять документацию категорий, порядка endpoints, повторов и фиксированного окна `lday=7`.
- [ ] Не обновлять `CLAUDE.md`: файл отсутствует, новые внутренние компоненты не вводятся.
