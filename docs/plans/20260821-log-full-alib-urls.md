...
- [ ] Заменить использование `endpointForLog` на полный `endpoint.String()` во всех download/parse событиях и контекстных ошибках страницы.
- [ ] Удалить ставший ненужным `endpointForLog`.
- [ ] Сохранить отдельную обработку `url.Error` и валидацию userinfo без изменений.
- [ ] Заменить тесты сокрытия query-параметров при download/parse failure тестами наличия полного URL.
- [ ] Обновить общий тест исходов страниц: проверить query-параметры в успешных и ошибочных download/parse событиях.
- [ ] Проверить, что агрегированная ошибка неуспешного fetch содержит URL соответствующих страниц с GET-параметрами.
...
- [ ] Обновить интеграционные проверки JSON-логов: каждый Alib event должен содержать исходный URL вместе с его query-параметрами.
- [ ] Удалить проверки отсутствия `scope`, `topic` и других GET-параметров в логах.
- [ ] Обновить README: атрибут `url` содержит полный настроенный адрес, включая GET-параметры.
- [ ] Обновить репозиторный контракт в AGENTS.md и убрать утверждения об исключении query-параметров из Alib-логов и ошибок.
