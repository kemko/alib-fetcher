...
- [x] Обновить многочастный service-тест: заголовок присутствует только в первом
  отправленном сообщении.
- [x] Сохранить проверки порядка книг, per-chunk acknowledgement, silent-флагов и кнопки `Обновить`
  только на последнем chunk.
- [x] Обновить README: при разбиении digest заголовок находится только в первом
  Telegram-сообщении.
- [x] Не менять конфигурацию, Telegram transport и формат отдельных книг.
...
- [x] Убедиться, что `README.md` точно описывает новое расположение заголовка.
- [x] Не обновлять `CLAUDE.md`: внутренние архитектурные паттерны не меняются.
