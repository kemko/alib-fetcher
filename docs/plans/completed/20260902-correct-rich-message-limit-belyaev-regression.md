...
- [x] Обновить стандартный `MESSAGE_LIMIT` до 32000 и допустимый диапазон до 64..32768.
- [x] Уточнить, что лимит применяется к рунам отображаемого Rich Message текста после разбора HTML.
- [x] Зафиксировать, что теги и URL в атрибутах не расходуют текстовый лимит, а переносы строк расходуют.
- [x] Сохранить правило сокращения только `Content` до максимального префикса с `…`.
- [x] Уточнить условие `ErrMessageTooLong` через отображаемую длину обязательных полей.
- [x] Повторно запустить `make verify`, `make coverage` и `docker compose config`.
- [x] Проверить итоговый diff, включить только относящиеся к задаче файлы и создать Conventional Commit.
